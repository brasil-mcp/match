...
def test_lookup_cnpj_is_callable():
    assert callable(brasil_mcp_match_server.lookup_cnpj)


def test_lookup_cnpj_delegates_to_postgres_repo():
    fake_record = MagicMock(cnpj_basico="33000167")
    with (
        patch("brasil_mcp_match_server.core.repository.connection.connect") as mock_connect,
        patch("brasil_mcp_match_server.core.repository.postgres_repo.PostgresCnpjRepo") as mock_repo_cls,
    ):
        mock_connect.return_value.__enter__.return_value = "fake_conn"
        mock_repo = mock_repo_cls.return_value
        mock_repo.find_by_cnpj.return_value = fake_record

        result = brasil_mcp_match_server.lookup_cnpj("33000167000101")
...
def test_lookup_cnpj_returns_none_when_not_found():
    with (
        ...
        result = brasil_mcp_match_server.lookup_cnpj("99999999000199")
