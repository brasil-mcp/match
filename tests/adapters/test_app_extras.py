...
from brasil_mcp_match.adapters.rest import app as app_mod
from brasil_mcp_match.adapters.rest.app import app, cli
...
def test_refresh_status_when_no_runs(service_context, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "brasil_mcp_match.core.ingestion.manifest.latest_successful",
        lambda: None,
    )
...
def test_refresh_status_when_populated(service_context, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from brasil_mcp_match.core.ingestion.manifest import IngestionRun
...
def test_cli_version_prints_version(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert out == brasil_mcp_match.__version__
...
def test_get_service_raises_when_unconfigured() -> None:
    ...
    """
    from brasil_mcp_match.adapters.rest import dependencies as deps_mod
...
def test_require_api_key_passes_through_auth_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from brasil_mcp_match.adapters.rest import dependencies as deps_mod
    from brasil_mcp_match.core.auth.api_key import AuthResult
    ...
    if saved is None:
        ...
        from brasil_mcp_match.adapters.rest.dependencies import ServiceContext
